...
if __name__ == "__main__":
    ...
    for idx in error_indices:
        ...
        while True:
            ...
            if noisy_y != ys_clean[idx]:
                ...
                break
    multiplicities = [4]*n  # 默认全1重数，后续根据噪声情况调整
    # for i in range(n):
    ...
    # # ===================== 1. GS译码参数 =====================
    # params = PHYParams()
    # c_exp = params.get("rs_c_exp")        # 有限域指数，默认GF(2^8)
    # k = params.get("rs_packet_size")
    # n = k + params.get("rs_nsym")
    ...
    # gf_rx, pad_bits = bits_to_gf(bits, GF)

    # if len(gf_rx) % n != 0:
    #     raise ValueError(f"编码符号长度必须是 {n} 的整数倍")
    # # 拆包
    # pkts = [gf_rx[i*n : (i+1)*n] for i in range(len(gf_rx) // n)]
    # multiplicities = [1] * n
    # decoded_pkts = []
    # for idx, pkt in enumerate(pkts):
    #     dec = decoder.decode(
    #         received_symbols=pkt,
    #         multiplicities=multiplicities,
    #         method="alekhnovich",    # 或 roth
    #         precision=None,
    #         # dc_threshold=3
    ...
    # gf_dec = GF(np.concatenate([p.view(np.ndarray) for p in decoded_pkts]))
    # decoded_bits = gf_to_bits(gf_dec)
